...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, in_dim, num_heads, window_size=7, shift_size=0,
...
def get_attention_mask(D,H,W,window_size, shift_size, device):
    ...
    Dp = int(np.ceil(D / window_size)) * window_size
    Hp = int(np.ceil(H / window_size)) * window_size
    Wp = int(np.ceil(W / window_size)) * window_size
    img_mask = torch.zeros((1, Dp, Hp, Wp, 1), device=device)  # 1 Hp Wp 1
    s_slices = (slice(0, window_size),
